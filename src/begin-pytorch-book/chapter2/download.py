...
def download_image(url, klass, data_type):
    ...
    if not os.path.exists(filename):
        try: 
            http = urllib3.PoolManager(retries=Retry(connect=3, read=3, redirect=3, backoff_factor=0.1))
            with http.request("GET", url, preload_content=False) as resp, open(
                ...
                if resp.status == 200:
                    ...
                    print("Error downloading {} code: {}".format(url, resp.status))
            resp.release_conn()
        except Exception as ex:
            print("Error downloading {} with exception {}".format(url, str(ex)))
